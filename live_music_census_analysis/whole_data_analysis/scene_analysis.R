####### Analysis of the whole data for "How is the live music scene in your city?" question #######
...
Q11_models_whole <- lapply(exp_vars, function(explanatory) {
  formula <- as.formula(paste("Q11", "~", explanatory))
  brm(formula, family = cumulative("logit"), data = whole_Q11, seed = 2025,
      cores = 4, control = list(adapt_delta = 0.99),
      prior = priors[[explanatory]], silent = 2, refresh = 0)
})

# Model comparison
loo(Q11_models_whole[[1]], Q11_models_whole[[2]],
    Q11_models_whole[[3]], Q11_models_whole[[4]])

# Calculating parameter effects and intervals
Q11_whole_draws <- expand.grid(Q2 = c("Female", "Male", "Another gender identity"),
                               Age_group = c("30-49", "18-29", "50-64", ">64"),
                               City = c("Helsinki", "Lviv", "Mannheim", "Vilnius",
                                        "Heidelberg")) %>%
  add_epred_draws(object = Q11_models_whole[[4]])
## Mean categories
### Gender
Q11_gender_mean_cate <- Q11_whole_draws %>%
  ungroup() %>%
  ...
  mutate(Q2 = case_match(Q2,
                         ...
                         "Female" ~ "Female"))
### Age group
Q11_age_group_mean_cate <- Q11_whole_draws %>%
  ungroup() %>%
  mutate(mean_cat = as.numeric(.category) * .epred) %>%
...
Q11_coef_estimates_b95_whole <- tidy(Q11_models_whole[[4]], conf.int = TRUE) %>%
...
Q11_coef_estimates_b50_whole <- tidy(Q11_models_whole[[4]],
...
Q11_coefs_whole <- Q11_coef_estimates_b95_whole %>%
  left_join(Q11_coef_estimates_b50_whole, by = "term") %>%
  mutate(Question = "Opinion on the scene")

# Calculating city estimates
Q11_city_estimates <- Q11_whole_draws %>%
...
### Gender
saveRDS(Q11_gender_mean_cate, file = "Q11_gender_mean_cate")
### Age group
saveRDS(Q11_age_group_mean_cate, file = "Q11_age_group_mean_cate")
# Coef estimates
saveRDS(Q11_coefs_whole, file = "Q11_coefs_whole")
# City estimates
saveRDS(Q11_city_estimates, file = "Q11_city_estimates")
